...
class TestRayOptimizer(unittest.TestCase):
    def setUp(self):
        ...
        self.evaluation_parameters = [[self.param_func() for _ in range(5)] for _ in range(4)]
    ...
    def test_loss_from_output(self):
        output, _ = RayOptimizer.run_engine(self.engine, self.evaluation_parameters, "ImagePlane", MultiLayer([0., 1., 2.]), True)
        target_rays = self.engine.run([self.param_func() for _ in range(5)], MultiLayer([0., 1., 2.]))
        criterion: RayLoss = BoxIoULoss('distance_box_iou_loss')
