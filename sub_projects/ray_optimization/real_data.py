        ...
        for file in files:
            if file.lower().endswith('.bmp') and not file.lower().endswith('black.bmp'):
                ...
                scatter = transform_weight(hist=image[:, cleaned_indices], pc_weights=cleaned_intensity.unsqueeze(0),
                                           num_rays=1000)
                ray_output = RayOutput(x_loc=torch.from_numpy(scatter[0, :, 0].float().numpy()), y_loc=torch.from_numpy(scatter[0, :, 1].float().numpy()),
                                       z_loc=torch.from_numpy(np.array([], dtype=float)), y_dir=torch.Tensor(np.array([], dtype=float)),
                                       x_dir=torch.from_numpy(np.array([], dtype=float)), z_dir=torch.from_numpy(np.array([], dtype=float)),
                                       energy=torch.from_numpy(np.array([], dtype=float)))
                z_layer_id = file[:-4]
